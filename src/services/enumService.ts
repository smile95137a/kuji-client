...
/** 前台 - 商品類別選項 GET /enums/lottery-category */
export const getLotteryCategoryOptions = async (
  req?: RequestData,
): Promise<ApiResponse<any>> => {
  try {
    const res = await api.get(`${basePath}/lottery-category`, {
      params: req ?? undefined,
    });
    return res.data;
  } catch (e) {
    console.error('Enums - getLotteryCategoryOptions error:', e);
    throw e;
  }
};
...
export const getLotterySubCategoryOptions = async (
  ...
  try {
    const res = await api.get(`${basePath}/lottery-sub-category`, {
      params: req ?? undefined,
    });
    return res.data;
  } catch (e) {
    console.error('Enums - getLotterySubCategoryOptions error:', e);
    throw e;
  }
};
